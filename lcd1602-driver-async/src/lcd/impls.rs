...
impl<'a, Sender, Delayer, const READABLE: bool> Basic for Lcd<'a, Sender, Delayer, READABLE>
where
    ...
    Delayer: DelayNs,
{
    ...
    async fn write_graph_to_cgram(&mut self, index: u8, graph_data: &CGRAMGraph) {
        ...
        let direction_flipped = if self.get_direction() == MoveDirection::RightToLeft {
            self.set_direction(MoveDirection::LeftToRight).await;
            true
        } else {
            false
        };
        ...
        for &line_data in &graph_data.upper {
            self.sender.wait_and_send(
        ...
        if let Some(graph_data_lower) = graph_data.lower {
            for &line_data in &graph_data_lower {
                self.sender.wait_and_send(
                    CommandSet::WriteDataToRAM(line_data),
                    &mut self.delayer,
                    self.poll_interval_us,
        ...
        if direction_flipped {
            self.set_direction(MoveDirection::RightToLeft).await;
        }
    }
    ...
    async fn return_home(&mut self) {
        ...
        self.sender
            .wait_and_send(CommandSet::ReturnHome, &mut self.delayer, self.poll_interval_us).await;
    }
    ...
    async fn delay_ms(&mut self, ms: u32) {
        self.delayer.delay_ms(ms).await;
    }

    async fn delay_us(&mut self, us: u32) {
        self.delayer.delay_us(us).await;
    }
}

impl<'a, Sender, Delayer> BasicRead for Lcd<'a, Sender, Delayer, true>
where
...
impl<'a, Sender, Delayer, const READABLE: bool> Ext for Lcd<'a, Sender, Delayer, READABLE>
where
    Delayer: DelayNs,
    Sender: SendCommand<Delayer, READABLE>,
{
...
impl<'a, Sender, Delayer> ExtRead for Lcd<'a, Sender, Delayer, true>
where
    Delayer: DelayNs,
    Sender: SendCommand<Delayer, true>,
{
...
impl<'a, Sender, Delayer, const READABLE: bool> Anim for Lcd<'a, Sender, Delayer, READABLE>
where
    Delayer: DelayNs,
    Sender: SendCommand<Delayer, READABLE>,
{
